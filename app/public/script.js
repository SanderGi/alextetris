...
const ratio =
  (200 * window.innerWidth) / 140 > window.innerHeight
...
document.getElementById("pause").style =
  "background-color: grey;position: absolute; top: 10px; left: " +
  (off.left + canvas.width - 20.5 * ratio) +
  "px;" +
  "padding: " +
  4 * ratio +
  "px" +
...
document.getElementById("playAgain").style =
  ...
  (off.top + canvas.height - 16 * ratio) +
  "px; left: " +
  (off.left + canvas.width / 2 - 34 * ratio) +
  "px; z-index: 100;" +
  "padding: " +
  4 * ratio +
  "px" +
...
function pause() {
  ...
  if (game.paused && getCookie("settings") !== "")
...
function GameOver() {
  ...
  DisplayLeaderboard(game.score).then((madeIt) => {
    // display leaderboard and enter player score if they make top 10
...
function loop(timeNew) {
  if (!game.gameOver) {
    const deltaTime = timeNew - timeOld;
...
let dista = 0;
ontouch(
  document.getElementById("touchSurface"),
  function (evt, dir, phase, swipetype, distance, elapsedtime) {
    ...
    let thres =
      (settings.touchSensitivity > 80
        ? 6
        : settings.touchSensitivity < 40
        ? 3
        : settings.touchSensitivity < 60
        ? 4
        : 5) * ratio;
    if (settings.touchSensitivity > 0 && !game.paused && !game.gameOver) {
      if (phase == "move" && dir == "right" && distance - dista >= 8 * ratio) {
        ...
        phase == "move" &&
        dir == "left" &&
        -distance + dista >= 8 * ratio
      ) {
        dista = distance;
        game.playerMove(-1, 0);
      } else if (
        phase == "end" &&
        (Math.abs(distance) < thres ||
          (Math.abs(dista) < thres && dir !== "up" && dir !== "down"))
      ) {
        ...
        dir == "down" &&
        elapsedtime < 800 &&
        (distance >= 450 - 5 * settings.touchSensitivity ||
          settings.touchSensitivity == 100)
      ) {
      ...
      game.isSoftDropping =
        phase == "move" && dir == "down" && elapsedtime > 450;
    }
  }
);
...
function draw() {
  ...
  if (settings.showPreview)
    drawPreview(game.player.matrix.data, game.player.x, game.previewHardDrop());
...
function DisplayPauseMenu() {
  ...
  menu.appendChild(
    createCheckable(
      settings.upswipeHold,
      "Up-swipe to Hold:",
      x,
      y + 8.5 * tile,
      9 * tile,
      function (a) {
        settings.upswipeHold = a;
      },
      8 * ratio,
      ""
    )
  ...
  menu.appendChild(
    createCheckable(
      settings.downSwipeHarddrop,
      "Down-swipe Drop:",
      x,
      y + 10 * tile,
      9 * tile,
      function (a) {
        settings.downSwipeHarddrop = a;
      },
      8 * ratio,
      ""
    )
...
function DisplayHold() {
  ...
  if (game.hold !== null) {
    const w = (ratio * 18) / game.hold.data[0].length;
    const h = (ratio * 18) / game.hold.data.length;
...
function drawScaledMatrix(matrix, x, y, w, h) {
  const tilew = (ratio * 18) / matrix[0].length;
  const tileh = (ratio * 18) / matrix.length;
...
async function OldDisplayLeaderboard(youScore) {
  ...
  for (
    let i = 0;
    i <
    Math.min(entries.length, 14) +
      (youScore > entries[Math.min(entries.length, 14) - 1].score ? 1 : 0);
    i++
  ) {
    const uname =
      !madeIt && youScore > entries[i].score
        ? "YOU"
        : entries[i - (madeIt ? 1 : 0)].username;
    const score =
      !madeIt && youScore > entries[i].score
        ? youScore
        : entries[i - (madeIt ? 1 : 0)].score;
    ...
    ctx.fillText(
      Math.round(score)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, " "),
      playX + playWidth * 8 * ratio - 0.1 * ratio,
      playY + 10 * ratio * (i + 2),
      playWidth * 3.8 * ratio
    );
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (youScore > entry.score && !madeIt) {
      ...
      container.appendChild(
        CreateRow(place + ". YOU", Format(youScore), "yellow")
      );
      place++;
    }
...
function continueSlide(speed, element) {
  ...
  setTimeout(() => {
    continueSlide(speed / 1.1, element);
  }, 50);
}
...
function CreateRow(text, score, color) {
  ...
  row.appendChild(
    CreateLabel(
      text,
      7 * ratio + "px sans-serif; float: left; width: 70%; color: " + color
    )
  );
  row.appendChild(
    CreateLabel(
      score,
      7 * ratio +
        "px sans-serif; float: left; width: 30%; text-align: right; color: " +
        color
    )
  );
  return row;
...
function CreateLabel(text, font) {
  ...
  element.style =
    "padding-top: 0px; padding-left: 0px;  text-overflow: ellipsis; white-space: nowrap; overflow: hidden; font: " +
    font;
  return element;
}
