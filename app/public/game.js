class Tetris {
  ...
  createQueue() {
    const tetrominos = ["T", "J", "L", "O", "S", "Z", "I"];
  ...
  holdCurrent() {
    if (this.holdUsed || this.gameOver || this.paused) {
      return;
    }
    if (this.hold == null) {
      ...
      const temp = this.hold.copy();
      this.hold = this.player.matrix.copy();
      this.player.matrix = temp;
      this.player.y = 0;
      this.player.x =
        this.width / 2 - Math.round(this.player.matrix.data[0].length / 2);
    }
    this.holdUsed = true;
  }
  ...
  update(deltaTime) {
    ...
    if (this.isSoftDropping) {
      this.dropInterval /= this.softSpeed;
      this.score += (2 * deltaTime) / this.dropInterval;
    }
  ...
  previewHardDrop() {
    ...
    let dropHeight = this.player.y;
    const temp = this.player.y;
    while (!this.collide()) {
      this.player.y++;
      dropHeight++;
    }
  ...
  playerRotate(dir, ignorAble) {
    if (this.gameOver || this.paused || (this.ignoreNextRotate && ignorAble)) {
      this.ignoreNextRotate = false;
      return;
    }
  ...
  mergePlayer() {
    ...
    for (let y = 0; y < this.player.matrix.data.length; y++) {
      for (let x = 0; x < this.player.matrix.data[0].length; x++) {
        if (this.player.matrix.data[y][x] !== 0) {
          this.world.data[y + this.player.y][x + this.player.x] =
            this.player.matrix.data[y][x];
        }
      }
    }
  }
  ...
  playerReset() {
    ...
    if (this.queue.length <= 3) {
      this.queue = this.queue.concat(this.createQueue());
    }
